...
export async function updateServer(config) {
  const spinner = createSpinner("Configuring server").start();

  try {
    await updateAppFile(config);
    await updateServerFile(config);

    spinner.succeed("Server configured");
  } catch (error) {
    spinner.fail("Failed to configure server");
    throw error;
  }
}
...
async function updateAppFile(config) {
  ...
  const appPath = path.join(config.targetDir, "src", `app.${extension}`);
...
async function updateServerFile(config) {
  ...
  const serverPath = path.join(config.targetDir, "src", `server.${extension}`);
  ...
  if (config.database && config.database !== "none") {
    ...
    if (!content.includes(dbImport)) {
      content = dbImport + "\n" + content;
    }

    if (!content.includes("connectDB();")) {
      content = content.replace("// Connect DB locally", "connectDB();");
    }
  }
