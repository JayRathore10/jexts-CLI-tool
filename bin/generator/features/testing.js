...
export async function setupTesting(config) {
  if (config.testing === "none" || !config.testing) {
    return;
  }

  const spinner = createSpinner(`Setting up ${config.testing}`).start();
  ...
  try {
    ...
    spinner.succeed(`${config.testing} configured`);
  } catch (error) {
    spinner.fail("Testing setup failed");
    throw error;
  }
}
