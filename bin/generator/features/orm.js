...
export async function setupORM(config) {
  if (config.orm === "none" || !config.orm) {
    return;
  }

  const spinner = createSpinner(`Setting up ${config.orm}`).start();
  ...
  try {
    ...
    spinner.succeed(`${config.orm} configured`);
  } catch (error) {
    spinner.fail(`${config.orm} setup failed`);
    throw error;
  }
}
